...
//       const parsed = lines.slice(1).map((line, i) => {
//         // Handle commas inside quotes if needed, or just standard split
//         const vals = line.split(',').map(v => v.trim())
//         const obj  = {}
//         headers.forEach((h, j) => obj[h] = vals[j] || '')
//         obj._row = i + 2
...
//         // 1b. Get section_id (optional, if provided in CSV)
//         let section_id = null;
//         if (row.section) {
//           const { data: sec } = await supabase.from('sections').select('id').eq('class_id', cls.id).ilike('name', row.section).single()
...
//         // ---------------------------------------------------------
//         // 3. Force Auto-generate UNIQUE roll number (Ignore CSV)
//         // ---------------------------------------------------------
//         let uid;
//         let exists = true;
//         while (exists) {
...
//               <span style={{ fontSize:'40px' }}>📄</span>
//               <p style={{ margin:'10px 0 4px', fontWeight:'700', color:'#374151' }}>Upload CSV File</p>
//               <p style={{ margin:'0 0 16px', fontSize:'13px', color:'#94a3b8' }}>Columns: name, email, password, class_name (roll_no will be auto-generated)</p>
//               <input type="file" accept=".csv" onChange={handleFile} style={{ display:'none' }} id="csv-input" />
//               <label htmlFor="csv-input" style={{ padding:'10px 24px', background:'#4f46e5', color:'#fff', borderRadius:'8px', cursor:'pointer', fontSize:'14px', fontWeight:'700' }}>
//                 Choose File
//               </label>
...
//               <table style={{ width:'100%', borderCollapse:'collapse' }}>
//                 <thead>
//                   <tr>{['Row','Name','Email','Class'].map(h=><th key={h} style={S.th}>{h}</th>)}</tr>
//                 </thead>
//                 <tbody>
//                   {rows.map((r,i)=>(
//                     <tr key={i}>
...
//                       <td style={{...S.td,fontWeight:'600'}}>{r.name}</td>
//                       <td style={S.td}>{r.email}</td>
//                       <td style={S.td}>{r.class_name} {r.section ? `- ${r.section}` : ''}</td>
//                     </tr>
//                   ))}
//                 </tbody>
//               </table>
...
//             <span style={{ fontSize:'40px' }}>⏳</span>
//             <p style={{ color:'#6366f1', fontWeight:'700', fontSize:'16px', margin:'12px 0 4px' }}>Importing students...</p>
//             <p style={{ color:'#94a3b8', fontSize:'13px' }}>Generating Roll Numbers and saving profiles...</p>
//           </div>
//         )}
...
//               <table style={{ width:'100%', borderCollapse:'collapse' }}>
//                 <thead>
//                   <tr>{['Name','Email','Roll No','Message'].map(h=><th key={h} style={S.th}>{h}</th>)}</tr>
//                 </thead>
//                 <tbody>
//                   {results.map((r,i)=>(
//                     <tr key={i}>
//                       <td style={{...S.td,fontWeight:'600'}}>{r.name}</td>
//                       <td style={S.td}>{r.email}</td>
//                       <td style={{...S.td, color: '#4f46e5', fontWeight: 'bold'}}>{r.roll_no || '-'}</td>
//                       <td style={{...S.td,fontSize:'12px',color: r.status==='success' ? '#16a34a' : '#ef4444'}}>
//                         {r.msg}
//                       </td>
//                     </tr>
//                   ))}
...
//     for (const row of rows) {
//       try {
//         // 1. SMART CLASS MATCHER: Handles "1" matching "1st" or "Class 1"
//         // const { data: cls, error: clsErr } = await supabase
//         //   .from('classes')
//         //   .select('id, name')
//         //   .or(`name.eq."${row.class_name}",name.ilike."${row.class_name}%"`)
//         //   .limit(1)
//         //   .single()

//         // This is the "Smart Matcher" logic now inside your runImport function:
// const { data: cls, error: clsErr } = await supabase
//   .from('classes')
//   .select('id, name')
//   // This looks for an exact match OR a name that starts with your number (e.g., "1" matches "1st")
//   .or(`name.eq."${row.class_name}",name.ilike."${row.class_name}%"`)
//   .limit(1)
//   .single()

//         if (clsErr || !cls) { 
//           res.push({ ...row, status:'error', msg:`Class "${row.class_name}" not found in DB` }); 
//           continue 
//         }
...
//         // 3. FORCE UNIQUE ROLL NUMBER (Ignore CSV Input)
//         let uid;
//         let exists = true;
//         while (exists) {
//           uid = generateRollNo()
...
//           <div>
//             <h3 style={{ margin:0, fontSize:'18px', fontWeight:'800', color:'#1e293b' }}>📥 Bulk CSV Import</h3>
//             <p style={{ margin:'3px 0 0', fontSize:'13px', color:'#64748b' }}>Automatic roll numbers will be assigned</p>
//           </div>
//           <button style={{ background:'none', border:'none', fontSize:'20px', cursor:'pointer', color:'#9ca3af' }} onClick={onClose}>✕</button>
//         </div>
...
//               <span style={{ fontSize:'40px' }}>📄</span>
//               <p style={{ margin:'10px 0 4px', fontWeight:'700', color:'#374151' }}>Upload CSV File</p>
//               <p style={{ margin:'0 0 16px', fontSize:'13px', color:'#94a3b8' }}>Required: name, email, password, class_name</p>
//               <input type="file" accept=".csv" onChange={handleFile} style={{ display:'none' }} id="csv-input" />
//               <label htmlFor="csv-input" style={{ padding:'10px 24px', background:'#4f46e5', color:'#fff', borderRadius:'8px', cursor:'pointer', fontSize:'14px', fontWeight:'700' }}>
//                 Choose File
//               </label>
...
//                       <td style={S.td}>{r.email}</td>
//                       <td style={S.td}>{r.class_name}</td>
//                       <td style={S.td}>{r.section || 'All'}</td>
//                     </tr>
//                   ))}
//                 </tbody>
//               </table>
...
//             <p style={{ color:'#6366f1', fontWeight:'700', fontSize:'16px', margin:'12px 0 4px' }}>Processing Students...</p>
//             <p style={{ color:'#94a3b8', fontSize:'13px' }}>Creating accounts and generating unique IDs</p>
//           </div>
//         )}
