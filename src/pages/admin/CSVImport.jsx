...
export default function CSVImport({ onClose, onDone }) {
  ...
  const generateRollNo = () => {
    const year = new Date().getFullYear().toString().slice(2)
    return '1' + year + (Math.floor(Math.random() * 9000) + 1000)
  }
  ...
  const runImport = async () => {
    ...
    // Pre-fetch all classes and sections once — avoids repeated DB calls
    const { data: allClasses } = await supabase.from('classes').select('id, name')
    const { data: allSections } = await supabase.from('sections').select('id, name, class_id')

    // Pre-fetch existing roll numbers to avoid collision
    const { data: existingRolls } = await supabase.from('students').select('roll_no')
    const usedRolls = new Set((existingRolls || []).map(r => String(r.roll_no)))

    const getUniqueRoll = () => {
      let uid
      do { uid = generateRollNo() } while (usedRolls.has(String(uid)))
      usedRolls.add(String(uid))
      return uid
    }
      ...
      const batchResults = await Promise.all(batch.map(async (row) => {
        try {
          ...
          const cls = allClasses?.find(c =>
            c.name === row.class_name.trim() ||
            c.name.toLowerCase().startsWith(row.class_name.trim().toLowerCase())
          )
          if (!cls) return { ...row, status: 'error', msg: `Class "${row.class_name}" not found` }

          // Find section
          let section_id = null
          if (row.section?.trim()) {
            const sec = allSections?.find(s =>
              s.class_id === cls.id &&
              s.name.toLowerCase() === row.section.trim().toLowerCase()
            )
            if (sec) section_id = sec.id
          }
          ...
          const authResult = await response.json()
          if (authResult.error) return { ...row, status: 'error', msg: authResult.error }

          const userId = authResult.user?.id
          if (!userId) return { ...row, status: 'error', msg: 'No user ID returned' }

          const uid = getUniqueRoll()
          ...
          const { error: pErr } = await supabase.from('profiles').insert({
            ...
            gender: row.gender?.toLowerCase().trim() || 'male',
            address: row.address?.trim() || null,
            birthday: row.dob?.trim() || null,
          })
          if (pErr) {
            await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/create-user`, {
              method: 'DELETE',
            ...
            return { ...row, status: 'error', msg: 'Profile error: ' + pErr.message }
          }

          // Create student
          const { error: sErr } = await supabase.from('students').insert({
            id: userId, student_id: uid, roll_no: uid,
            class_id: cls.id, section_id
          })
          if (sErr) return { ...row, status: 'error', msg: 'Student error: ' + sErr.message }

          return { ...row, roll_no: uid, status: 'success', msg: 'Created successfully' }
        } catch (e) {
          return { ...row, status: 'error', msg: e.message }
        }
      }))
    ...
    if (profile?.id) {
      await logActivity({
        ...
        action: 'CSV Import', target_type: 'students',
        target_name: `${successCount} students`,
        details: `Imported ${successCount}/${rows.length} students`
      })
    }
      ...
      <div style={S.modal} onClick={e => e.stopPropagation()}>
        ...
        {step === 'upload' && (
          <div>
            <div style={{ background: '#f8fafc', border: '2px dashed #e2e8f0', borderRadius: '12px', padding: '30px', textAlign: 'center', marginBottom: '16px' }}>
              <span style={{ fontSize: '40px' }}>📄</span>
              <p style={{ margin: '10px 0 4px', fontWeight: '700', color: '#374151' }}>Upload CSV File</p>
              <p style={{ margin: '0 0 6px', fontSize: '13px', color: '#94a3b8' }}>Required: <strong>name, email, password, class_name</strong></p>
              <p style={{ margin: '0 0 16px', fontSize: '12px', color: '#94a3b8' }}>Optional: phone, gender, section, address, dob</p>
              <input type="file" accept=".csv" onChange={handleFile} style={{ display: 'none' }} id="csv-input" />
              <label htmlFor="csv-input" style={{ padding: '10px 24px', background: '#4f46e5', color: '#fff', borderRadius: '8px', cursor: 'pointer', fontSize: '14px', fontWeight: '700' }}>
                Choose File
        ...
        {step === 'preview' && (
          <div>
            ...
            <div style={{ maxHeight: '300px', overflowY: 'auto', border: '1px solid #e2e8f0', borderRadius: '8px' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead><tr>{['Name', 'Email', 'Class', 'Section'].map(h => <th key={h} style={S.th}>{h}</th>)}</tr></thead>
                <tbody>
                  {rows.map((r, i) => (
                    <tr key={i}>
                      <td style={{ ...S.td, fontWeight: '600' }}>{r.name}</td>
        ...
        {step === 'importing' && (
          <div style={{ textAlign: 'center', padding: '40px' }}>
            ...
            <p style={{ color: '#6366f1', fontWeight: '700', fontSize: '16px', margin: '0 0 8px' }}>Importing Students...</p>
            <p style={{ color: '#94a3b8', fontSize: '13px', margin: '0 0 20px' }}>{progress.done} / {progress.total} processed</p>
            <div style={{ backgroundColor: '#f1f5f9', borderRadius: '99px', height: '8px', overflow: 'hidden' }}>
              <div style={{ height: '100%', backgroundColor: '#4f46e5', borderRadius: '99px', width: progress.total ? `${(progress.done / progress.total) * 100}%` : '0%', transition: 'width 0.3s' }} />
            </div>
          </div>
        )}
        ...
        {step === 'done' && (
          <div>
            ...
            <div style={{ maxHeight: '280px', overflowY: 'auto', border: '1px solid #e2e8f0', borderRadius: '8px' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead><tr>{['Name', 'Roll No', 'Status', 'Note'].map(h => <th key={h} style={S.th}>{h}</th>)}</tr></thead>
                <tbody>
                  {results.map((r, i) => (
                    <tr key={i}>
                      <td style={{ ...S.td, fontWeight: '600' }}>{r.name}</td>
...
const S = {
  overlay: { position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.5)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '20px' },
  modal: { background: '#fff', borderRadius: '16px', padding: '28px', width: '100%', maxWidth: '640px', maxHeight: '90vh', overflow: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.1)' },
  th: { padding: '10px 14px', fontSize: '11px', fontWeight: '700', color: '#64748b', textTransform: 'uppercase', background: '#f8fafc', textAlign: 'left', borderBottom: '1px solid #f1f5f9' },
  td: { padding: '10px 14px', fontSize: '13px', color: '#334155', borderBottom: '1px solid #f8fafc' },
  btnPrimary: { padding: '12px 22px', background: '#4f46e5', color: '#fff', border: 'none', borderRadius: '8px', fontSize: '14px', fontWeight: '700', cursor: 'pointer' },
  btnGhost: { padding: '10px 18px', background: '#f1f5f9', color: '#475569', border: 'none', borderRadius: '8px', fontSize: '13px', fontWeight: '600', cursor: 'pointer' },
}