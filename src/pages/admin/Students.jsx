...
//   const [form, setForm] = useState({
//     name: '', email: '', password: '', phone: '', gender: 'male',
//     birthday: '', address: '', class_id: '', section_id: '', roll_no: ''
//   })

//   useEffect(() => {
//     getUniqueRollNo().then(r => setForm(prev => ({ ...prev, roll_no: r })))
//     supabase.from('classes').select('*').then(({ data }) => setClasses(data || []))
//   }, [])
...
//     setMessage(`✅ Student added! Student ID: ${studentId}`)
//     const nextRoll = await getUniqueRollNo()
//     setForm({ name: '', email: '', password: '', phone: '', gender: 'male', birthday: '', address: '', class_id: '', section_id: '', roll_no: nextRoll })
//     setLoading(false)
//   }
...
//           <p style={{ fontSize: '13px', color: '#6b7280', margin: 0 }}>Fill in the form below to add a student</p>
//         </div>
//         <button onClick={() => setShowCSV(true)} style={styles.csvBtn}>📥 Import CSV</button>
//       </div>
...
//               <div style={{ display: 'flex', gap: '8px', marginTop: '6px' }}>
//                 <div style={styles.idBadge}>{form.roll_no}</div>
//                 <button type="button" style={styles.refreshBtn} onClick={async () => { const r = await getUniqueRollNo(); setForm({ ...form, roll_no: r }) }}>🔄 Regenerate</button>
//               </div>
//             </div>
//             <div style={styles.formGrid}>
//               {[
...
//                 <div key={field.key} style={styles.formGroup}>
//                   <label style={styles.label}>{field.label}</label>
//                   <input style={styles.input} type={field.type} placeholder={field.placeholder} value={form[field.key]} onChange={e => setForm({ ...form, [field.key]: e.target.value })} />
//                 </div>
//               ))}
//               <div style={styles.formGroup}>
//                 <label style={styles.label}>Gender</label>
...
//           {message && (
//             <div style={{ ...styles.messageBox, backgroundColor: message.startsWith('✅') ? '#f0fdf4' : '#fef2f2', borderColor: message.startsWith('✅') ? '#86efac' : '#fca5a5', color: message.startsWith('✅') ? '#16a34a' : '#dc2626' }}>{message}</div>
//           )}

//           <div style={{ display: 'flex', gap: '12px', marginTop: '8px' }}>
//             <button style={styles.submitBtn} onClick={handleAdd} disabled={loading}>{loading ? '⏳ Adding Student...' : '✅ Add Student'}</button>
//             <button style={styles.cancelBtn} onClick={async () => { const r = await getUniqueRollNo(); setForm({ name: '', email: '', password: '', phone: '', gender: 'male', birthday: '', address: '', class_id: '', section_id: '', roll_no: r }); setMessage('') }}>Reset</button>
//           </div>
//         </div>
//       </div>
//       {showCSV && <CSVImport onClose={() => setShowCSV(false)} onDone={() => setShowCSV(false)} />}
...
//   const [deleteId, setDeleteId] = useState(null)
//   const [page, setPage] = useState(1)
//   const [globalStats, setGlobalStats] = useState({ total: 0, boys: 0, girls: 0 })
...
//     const { data } = await supabase.from('profiles').select('gender').eq('role', 'student')
//     if (data) {
//       setGlobalStats({
//         total: data.length,
//         boys: data.filter(d => d.gender === 'male').length,
//         girls: data.filter(d => d.gender === 'female').length
//       })
//     }
//   }
...
//     setLoading(true)
//     setSearched(true)
//     let query = supabase.from('students').select('*, profiles(name, uid, email, phone, gender, birthday, address), classes(name), sections(name)')
//     if (classId) query = query.eq('class_id', classId)
//     if (sectionId) query = query.eq('section_id', sectionId)
//     const { data } = await query
//     setStudents(data || [])
...
//       supabase.from('fee_assignments').delete().eq('student_id', id),
//       supabase.from('fee_payments').delete().eq('student_id', id),
//       supabase.from('feedback').delete().eq('student_id', id),
//       supabase.from('focus_logs').delete().eq('student_id', id),
//       supabase.from('assignment_analysis').delete().eq('student_id', id),
...
//   const handleEditOpen = async (s) => {
//     setEditStudent(JSON.parse(JSON.stringify(s)))
//     if (s.class_id) {
//       const { data } = await supabase.from('sections').select('*').eq('class_id', s.class_id)
//       setEditSections(data || [])
//     }
...
//     const updatedS = {
//       ...editStudent,
//       classes: classes.find(c => String(c.id) === String(editStudent.class_id)),
//       sections: editSections.find(s => String(s.id) === String(editStudent.section_id))
//     }
//     setStudents(students.map(s => s.id === editStudent.id ? updatedS : s))
//     setEditStudent(null)
//     fetchGlobalStats()
//   }
...
//           <span style={localStyles.statLabel}>Database Students</span>
//           <span style={localStyles.statValue}>{globalStats.total}</span>
//         </div>
//         <div style={localStyles.statItem}>
//           <span style={localStyles.statLabel}>Boys</span>
//           <span style={{...localStyles.statValue, color: '#2563eb'}}>{globalStats.boys}</span>
//         </div>
//         <div style={localStyles.statItem}>
//           <span style={localStyles.statLabel}>Girls</span>
//           <span style={{...localStyles.statValue, color: '#db2777'}}>{globalStats.girls}</span>
//         </div>
//       </div>
...
//         <div style={styles.emptyState}>
//           <div style={{ fontSize: '56px', marginBottom: '12px' }}>🎓</div>
//           <div style={{ fontWeight: '600', color: '#374151' }}>Search for students</div>
//         </div>
//       ) : (
//         <>
//           <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
//             <h3 style={{ fontSize: '15px', fontWeight: '700', color: '#1a1a2e' }}>
//               Showing <span style={{ color: '#4f46e5' }}>{filtered.length}</span> results
//             </h3>
//             <input style={{ ...styles.input, width: '220px' }} placeholder="Search name or roll no..." value={search} onChange={e => { setSearch(e.target.value); setPage(1) }} />
//           </div>
...
//               <div key={s.id} style={styles.studentCard}>
//                 <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px' }}>
//                   <div style={{ ...styles.avatar, backgroundColor: avatarColors[i % avatarColors.length] }}>{s.profiles?.name?.charAt(0).toUpperCase()}</div>
//                   <div style={{ flex: 1, minWidth: 0 }}>
//                     <div style={{ fontWeight: '700', fontSize: '14px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{s.profiles?.name}</div>
//                     <div style={{ fontSize: '12px', color: '#6b7280' }}>Roll No: {s.roll_no}</div>
//                   </div>
//                 </div>
//                 <div style={{ marginBottom: '12px' }}>
//                   <span style={styles.classPill}>🏫 {s.classes?.name || 'No Class'}{s.sections?.name ? ` - ${s.sections.name}` : ''}</span>
//                 </div>
//                 <div style={{ display: 'flex', gap: '8px', borderTop: '1px solid #f3f4f6', paddingTop: '10px' }}>
//                   <button style={styles.cardEditBtn} onClick={() => handleEditOpen(s)}>✏️ Edit</button>
//                   <button style={styles.cardDeleteBtn} onClick={() => setDeleteId(s.id)}>🗑 Delete</button>
...
//                     {index > 0 && array[index - 1] !== p - 1 && <span style={{ margin: '0 8px', color: '#9ca3af' }}>...</span>}
//                     <button style={{ ...styles.pageBtn, ...(p === page ? styles.pageActive : {}) }} onClick={() => setPage(p)}>{p}</button>
//                   </div>
//                 ))}
//               <button style={styles.pageBtn} onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages}>Next →</button>
//             </div>
...
//         <div style={styles.modalOverlay}>
//           <div style={{ ...styles.modal, width: '580px', maxHeight: '90vh', overflowY: 'auto' }}>
//             <h3 style={{ fontSize: '18px', fontWeight: '700', marginBottom: '20px' }}>Edit Student</h3>
//             <div style={styles.formGrid}>
//               <div style={styles.formGroup}><label style={styles.label}>Name</label><input style={styles.input} value={editStudent.profiles?.name || ''} onChange={e => setEditStudent({...editStudent, profiles: {...editStudent.profiles, name: e.target.value}})} /></div>
//               <div style={styles.formGroup}><label style={styles.label}>Email</label><input style={styles.input} value={editStudent.profiles?.email || ''} onChange={e => setEditStudent({...editStudent, profiles: {...editStudent.profiles, email: e.target.value}})} /></div>
//             </div>
//             <div style={{ display: 'flex', gap: '12px', marginTop: '20px' }}>
//               <button style={styles.cancelBtn} onClick={() => setEditStudent(null)}>Cancel</button>
//               <button style={styles.submitBtn} onClick={handleEdit}>Save</button>
//             </div>
//           </div>
//         </div>
//       )}
...
//         <div style={styles.modalOverlay}>
//           <div style={{ ...styles.modal, width: '360px', textAlign: 'center' }}>
//             <h3 style={{ fontSize: '18px', fontWeight: '700' }}>Delete Student?</h3>
//             <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', marginTop: '20px' }}>
//               <button style={styles.cancelBtn} onClick={() => setDeleteId(null)}>Cancel</button>
//               <button style={{ ...styles.submitBtn, backgroundColor: '#ef4444' }} onClick={confirmDelete}>Delete</button>
//             </div>
//           </div>
//         </div>
//       )}
...
//     setLoading(true)
//     setSearched(true)
//     const { data } = await supabase.from('feedback').select('*, profiles(name)').eq('student_id', selectedStudent).order('created_at', { ascending: false })
//     setFeedbacks(data || [])
//     setLoading(false)
//   }
...
//           <div style={styles.filterGroup}><label style={styles.label}>Class</label><select style={styles.input} value={selectedClass} onChange={e => handleClassChange(e.target.value)}><option value="">-- Class --</option>{classes.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
//           <div style={styles.filterGroup}><label style={styles.label}>Student</label><select style={styles.input} value={selectedStudent} onChange={e => setSelectedStudent(e.target.value)}><option value="">-- Student --</option>{students.map(s => <option key={s.id} value={s.id}>{s.profiles?.name}</option>)}</select></div>
...
//             <input style={{ ...styles.input, flex: 1 }} placeholder="Feedback message..." value={newFeedback} onChange={e => setNewFeedback(e.target.value)} />
//             <button style={styles.submitBtn} onClick={sendFeedback}>Send</button>
//           </div>
//           {feedbacks.map(f => (
//             <div key={f.id} style={styles.feedbackItem}><p style={{ margin: 0, fontSize: '14px' }}>{f.message}</p></div>
//           ))}
//         </div>
//       )}
//     </div>
...
// const localStyles = {
//   statsBar: { display: 'flex', gap: '32px', backgroundColor: '#f8fafc', padding: '16px 24px', borderRadius: '12px', border: '1px solid #e2e8f0', marginBottom: '20px' },
//   statItem: { display: 'flex', flexDirection: 'column' },
//   statLabel: { fontSize: '11px', fontWeight: '700', color: '#64748b', textTransform: 'uppercase' },
...
//   input: { padding: '10px 12px', borderRadius: '8px', border: '1px solid #e5e7eb', fontSize: '14px', outline: 'none', backgroundColor: '#fff' },
//   idBadge: { padding: '10px 16px', backgroundColor: '#ede9fe', color: '#6d28d9', borderRadius: '8px', fontSize: '18px', fontWeight: '800', border: '1px solid #ddd6fe' },
//   refreshBtn: { padding: '8px 14px', backgroundColor: '#e5e7eb', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '13px' },
//   submitBtn: { padding: '11px 24px', borderRadius: '8px', border: 'none', backgroundColor: '#4f46e5', color: '#fff', cursor: 'pointer', fontSize: '14px', fontWeight: '600' },
//   cancelBtn: { padding: '10px 20px', borderRadius: '8px', border: '1px solid #e5e7eb', background: '#fff', cursor: 'pointer', fontSize: '14px' },
//   messageBox: { padding: '12px 16px', borderRadius: '8px', border: '1px solid', fontSize: '14px', marginBottom: '8px' },
//   csvBtn: { padding: '9px 18px', backgroundColor: '#0ea5e9', color: '#fff', border: 'none', borderRadius: '8px', fontSize: '13px', fontWeight: '700', cursor: 'pointer' },
//   filterBox: { backgroundColor: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: '10px', padding: '16px', marginBottom: '20px' },
//   filterGroup: { display: 'flex', flexDirection: 'column', gap: '6px', minWidth: '160px' },
//   findBtn: { padding: '10px 20px', backgroundColor: '#4f46e5', color: '#fff', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '14px', fontWeight: '600' },
//   studentCard: { backgroundColor: '#fff', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '16px' },
//   avatar: { width: '44px', height: '44px', borderRadius: '50%', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px', fontWeight: '700' },
//   classPill: { backgroundColor: '#dbeafe', color: '#1e40af', padding: '3px 10px', borderRadius: '20px', fontSize: '12px', fontWeight: '500' },
//   cardEditBtn: { flex: 1, padding: '7px', backgroundColor: '#22c55e', color: '#fff', border: 'none', borderRadius: '7px', cursor: 'pointer', fontSize: '12px', fontWeight: '600' },
//   cardDeleteBtn: { flex: 1, padding: '7px', backgroundColor: '#ef4444', color: '#fff', border: 'none', borderRadius: '7px', cursor: 'pointer', fontSize: '12px', fontWeight: '600' },
//   pagination: { display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '6px', marginTop: '24px', flexWrap: 'nowrap' },
//   pageBtn: { padding: '7px 14px', borderRadius: '7px', border: '1px solid #e5e7eb', background: '#fff', cursor: 'pointer', fontSize: '13px' },
//   pageActive: { backgroundColor: '#4f46e5', color: '#fff', border: '1px solid #4f46e5' },
//   modalOverlay: { position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 },
//   modal: { backgroundColor: '#fff', borderRadius: '16px', padding: '28px' },
//   emptyState: { textAlign: 'center', padding: '60px 0', color: '#9ca3af', fontSize: '14px' },
//   feedbackItem: { backgroundColor: '#f9fafb', borderRadius: '8px', padding: '12px 16px', borderLeft: '4px solid #4f46e5', marginBottom: '8px' },
// }
...
function AddStudents() {
  ...
  const [classes, setClasses] = useState([])
  const [sections, setSections] = useState([])
  const generateRollNo = () => { const year = new Date().getFullYear().toString().slice(2); return '1' + year + (Math.floor(Math.random() * 9000) + 1000) }
  const getUniqueRollNo = async () => {
    let roll_no, exists = true
    while (exists) {
      roll_no = generateRollNo()
  ...
  const [form, setForm] = useState({ name: '', email: '', password: '', phone: '', gender: 'male', birthday: '', address: '', class_id: '', section_id: '', roll_no: generateRollNo() })
  const [loading, setLoading] = useState(false)
  const [showCSV, setShowCSV] = useState(false)
  const [message, setMessage] = useState('')
  ...
  const handleAdd = async () => {
    ...
    const userId = authResult.user?.id
    const { error: profileError } = await supabase.from('profiles').insert({
      id: userId, uid: roll_no, role: 'student',
      name: form.name, email: form.email, phone: form.phone || null,
      gender: form.gender, birthday: form.birthday || null, address: form.address || null,
    })
    if (profileError) { setMessage(profileError.message); setLoading(false); return }
    await supabase.from('students').insert({ id: userId, student_id: roll_no, class_id: form.class_id || null, section_id: form.section_id || null, roll_no })
    setMessage(`✅ Student added! Student ID: ${roll_no}`)
    await logActivity({ performed_by: profile.id, performed_by_name: profile.name, role: 'admin', action: 'Student Created', target_type: 'student', target_name: form.name, details: `Student ID: ${roll_no}` })
    const newRoll = await getUniqueRollNo()
    setForm({ name: '', email: '', password: '', phone: '', gender: 'male', birthday: '', address: '', class_id: '', section_id: '', roll_no: newRoll })
    setSections([])
    setLoading(false)
  }
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: '28px', alignItems: 'flex-start' }}>
        <div style={{ flex: 1 }}>
          <div style={styles.formSection}>
            <div style={styles.formSectionHeader}><span style={styles.formSectionIcon}>👤</span><h3 style={styles.formSectionTitle}>Basic Information</h3></div>
            <div style={{ marginBottom: '16px' }}>
              <label style={styles.label}>Roll No (Auto-Generated)</label>
              <div style={{ display: 'flex', gap: '8px', marginTop: '6px' }}>
                <div style={styles.idBadge}>{form.roll_no}</div>
                <button type="button" style={styles.refreshBtn} onClick={async () => { const r = await getUniqueRollNo(); setForm(f => ({ ...f, roll_no: r })) }}>🔄 Regenerate</button>
              </div>
            </div>
            <div style={styles.formGrid}>
              {[
                ...
                <div key={field.key} style={styles.formGroup}>
                  <label style={styles.label}>{field.label}</label>
                  <input style={styles.input} type={field.type} placeholder={field.placeholder} value={form[field.key]} onChange={e => setForm(f => ({ ...f, [field.key]: e.target.value }))} />
                </div>
              ))}
              <div style={styles.formGroup}>
                <label style={styles.label}>Gender</label>
          ...
          <div style={styles.formSection}>
            <div style={styles.formSectionHeader}><span style={styles.formSectionIcon}>🏫</span><h3 style={styles.formSectionTitle}>Class & Section</h3></div>
            <div style={styles.formGrid}>
              <div style={styles.formGroup}>
                <label style={styles.label}>Class</label>
                <select style={styles.input} value={form.class_id} onChange={e => handleClassChange(e.target.value)}>
              ...
              <div style={styles.formGroup}>
                <label style={styles.label}>Section</label>
                <select style={styles.input} value={form.section_id} onChange={e => setForm(f => ({ ...f, section_id: e.target.value }))}>
                  <option value="">-- Select Section --</option>
                  {sections.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                </select>
              </div>
          ...
          <div style={{ display: 'flex', gap: '12px', marginTop: '8px' }}>
            <button style={styles.submitBtn} onClick={handleAdd} disabled={loading}>{loading ? '⏳ Adding...' : '✅ Add Student'}</button>
            <button style={styles.cancelBtn} onClick={async () => { const r = await getUniqueRollNo(); setForm({ name: '', email: '', password: '', phone: '', gender: 'male', birthday: '', address: '', class_id: '', section_id: '', roll_no: r }); setMessage('') }}>Reset</button>
          </div>
        </div>
        <div style={styles.tipsPanel}>
          <h4 style={styles.tipsPanelTitle}>💡 Quick Guide</h4>
          {['Fill in basic info — name, email, and password are required.', 'Roll No is auto-generated and used as the Student ID for login.', 'Assign the correct class and section for proper management.', 'Use CSV Import to add multiple students at once.'].map((tip, i) => (
            <div key={i} style={styles.tipItem}><span style={styles.tipNum}>{i + 1}</span><span>{tip}</span></div>
          ))}
          <div style={{ marginTop: '20px', padding: '12px', backgroundColor: '#fffbeb', borderRadius: '8px', border: '1px solid #fde68a' }}>
            <p style={{ fontSize: '12px', color: '#92400e', margin: 0 }}>⚠️ The student uses their Roll No and password to log in — not email.</p>
          </div>
...
function ShowStudents() {
  ...
  const fetchStudents = async (classId, sectionId) => {
    ...
    let query = supabase.from('students').select('*, profiles(id, name, uid, email, phone, gender, birthday, address), classes(name), sections(name)')
    if (classId) query = query.eq('class_id', classId)
    if (sectionId) query = query.eq('section_id', sectionId)
    const { data } = await query
    setStudents(data || [])
  ...
  const confirmDelete = async () => {
    ...
    await supabase.from('profiles').delete().eq('id', id)
    setStudents(prev => prev.filter(s => s.id !== id))
    await logActivity({ performed_by: profile.id, performed_by_name: profile.name, role: 'admin', action: 'Student Deleted', target_type: 'student', target_name: student?.profiles?.name || 'Unknown', details: 'Student removed from system' })
  }
  ...
  const handleEdit = async () => {
    ...
    const { error: pErr } = await supabase.from('profiles').update({
      ...
      gender: editStudent.profiles?.gender,
      birthday: editStudent.profiles?.birthday || null,
      address: editStudent.profiles?.address || null,
      uid: editStudent.roll_no,
    }).eq('id', id)
    if (pErr) { alert('Save failed: ' + pErr.message); return }
    ...
    if (sErr) { alert('Save failed: ' + sErr.message); return }

    // Refetch to get fresh joined data
    const { data: updated } = await supabase
      .from('students')
      .select('*, profiles(id, name, uid, email, phone, gender, birthday, address), classes(name), sections(name)')
      .eq('id', id)
      .single()

    if (updated) setStudents(prev => prev.map(s => s.id === id ? updated : s))
    setEditStudent(null)
  }
  ...
  // Smart pagination: show first, last, and window around current
  ...
  return (
    <div>
      ...
      {!searched ? (
        <div style={styles.emptyState}>
          <div style={{ fontSize: '56px', marginBottom: '12px' }}>🎓</div>
          <div style={{ fontWeight: '600', color: '#374151', fontSize: '16px' }}>Search for students</div>
          <div style={{ fontSize: '13px', color: '#9ca3af', marginTop: '4px' }}>Select a class and section, or click "All Students"</div>
        </div>
      ) : (
        <>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
            ...
            <input style={{ ...styles.input, width: '220px' }} placeholder="Search by name or roll no..." value={search} onChange={e => { setSearch(e.target.value); setPage(1) }} />
          </div>

          {loading ? (
            <div style={styles.emptyState}>⏳ Loading students...</div>
          ) : paginated.length === 0 ? (
            <div style={styles.emptyState}><div style={{ fontSize: '48px' }}>🎓</div><div style={{ fontWeight: '600', color: '#374151' }}>No students found</div></div>
          ) : (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '14px' }}>
              {paginated.map((s, i) => {
                const color = avatarColors[i % avatarColors.length]
                ...
                return (
                  <div key={s.id} style={styles.studentCard}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px' }}>
                      <div style={{ ...styles.avatar, backgroundColor: color }}>{s.profiles?.name?.charAt(0).toUpperCase()}</div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontWeight: '700', fontSize: '14px', color: '#1a1a2e', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{s.profiles?.name}</div>
                        <div style={{ fontSize: '12px', color: '#6b7280' }}>Roll No: {s.roll_no}</div>
                      </div>
                    ...
                    <div style={{ marginBottom: '12px' }}>
                      {s.classes?.name ? (
                        <span style={styles.classPill}>🏫 {s.classes.name}{s.sections?.name ? ` - ${s.sections.name}` : ''}</span>
                      ) : (
                        <span style={{ fontSize: '12px', backgroundColor: '#fffbeb', color: '#92400e', padding: '3px 10px', borderRadius: '20px' }}>⚠️ No class assigned</span>
                      )}
                    </div>
            ...
            <div style={styles.pagination}>
              ...
              {getPageNumbers().map((p, idx, arr) => (
                <span key={p} style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                  {idx > 0 && arr[idx - 1] !== p - 1 && <span style={{ color: '#9ca3af', fontSize: '13px' }}>…</span>}
                  <button style={{ ...styles.pageBtn, ...(p === page ? styles.pageActive : {}) }} onClick={() => setPage(p)}>{p}</button>
                </span>
              ))}
              <button style={styles.pageBtn} onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages}>Next →</button>
        ...
        <div style={styles.modalOverlay}>
          <div style={{ ...styles.modal, width: '580px', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={styles.formGrid}>
              {[
                ...
                <div key={f.key} style={styles.formGroup}>
                  ...
                  <input style={styles.input} type={f.type || 'text'} value={editStudent.profiles?.[f.key] || ''}
                    onChange={e => setEditStudent(prev => ({ ...prev, profiles: { ...prev.profiles, [f.key]: e.target.value } }))} />
                </div>
              ))}
              <div style={styles.formGroup}>
                <label style={styles.label}>Gender</label>
                <select style={styles.input} value={editStudent.profiles?.gender || 'male'}
                  onChange={e => setEditStudent(prev => ({ ...prev, profiles: { ...prev.profiles, gender: e.target.value } }))}>
                  <option value="male">Male</option><option value="female">Female</option><option value="other">Other</option>
                </select>
              </div>
              <div style={styles.formGroup}>
                <label style={styles.label}>Birthday</label>
                <input style={styles.input} type="date" value={editStudent.profiles?.birthday?.slice(0, 10) || ''}
                  onChange={e => setEditStudent(prev => ({ ...prev, profiles: { ...prev.profiles, birthday: e.target.value } }))} />
              </div>
              <div style={styles.formGroup}>
                <label style={styles.label}>Roll No</label>
                <input style={{ ...styles.input, backgroundColor: '#f9fafb', color: '#6b7280' }} value={editStudent.roll_no} readOnly />
            ...
            <div style={{ ...styles.formGroup, marginTop: '12px' }}>
              <label style={styles.label}>Address</label>
              <input style={styles.input} value={editStudent.profiles?.address || ''}
                onChange={e => setEditStudent(prev => ({ ...prev, profiles: { ...prev.profiles, address: e.target.value } }))} />
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginTop: '12px' }}>
              <div style={styles.formGroup}>
                ...
                <select style={styles.input} value={editStudent.class_id || ''}
                  onChange={async e => {
                    ...
                    const { data } = await supabase.from('sections').select('*').eq('class_id', cid)
                    setEditSections(data || [])
                    setEditStudent(prev => ({ ...prev, class_id: cid, section_id: '' }))
                  }}>
                  <option value="">-- Select Class --</option>
                  {classes.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                </select>
              ...
              <div style={styles.formGroup}>
                <label style={styles.label}>Section</label>
                <select style={styles.input} value={editStudent.section_id || ''}
                  onChange={e => setEditStudent(prev => ({ ...prev, section_id: e.target.value }))}>
                  <option value="">-- Select Section --</option>
                  {editSections.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                </select>
              </div>
      ...
      {deleteId && (
        <div style={styles.modalOverlay}>
          <div style={{ ...styles.modal, width: '360px', textAlign: 'center' }}>
            <div style={{ fontSize: '52px', marginBottom: '8px' }}>⚠️</div>
            <h3 style={{ fontSize: '18px', fontWeight: '700', color: '#1a1a2e', margin: '0 0 8px' }}>Delete Student?</h3>
            <p style={{ color: '#6b7280', fontSize: '14px', margin: '0 0 20px' }}>This will permanently remove the student and all their data.</p>
            <div style={{ display: 'flex', gap: '12px', justifyContent: 'center' }}>
              <button style={styles.cancelBtn} onClick={() => setDeleteId(null)}>Cancel</button>
              <button style={{ ...styles.submitBtn, backgroundColor: '#ef4444' }} onClick={confirmDelete}>Yes, Delete</button>
            </div>
...
function StudentFeedback() {
  ...
  const handleFind = async () => {
    ...
    setLoading(true); setSearched(true)
    const { data } = await supabase.from('feedback').select('*, profiles(name)').eq('student_id', selectedStudent).order('created_at', { ascending: false })
    setFeedbacks(data || []); setLoading(false)
  }
  const sendFeedback = async () => {
    if (!newFeedback.trim() || !selectedStudent) return
    const { data: { user } } = await supabase.auth.getUser()
    const { data } = await supabase.from('feedback').insert({ student_id: selectedStudent, class_id: selectedClass || null, section_id: selectedSection || null, message: newFeedback.trim(), sent_by: user.id }).select('*, profiles(name)').single()
    if (data) { setFeedbacks([data, ...feedbacks]); setNewFeedback('') }
  }
  const selectedStudentName = students.find(s => s.id === selectedStudent)?.profiles?.name
  ...
  return (
    <div>
      ...
      <div style={styles.filterBox}>
        <div style={{ display: 'flex', gap: '16px', alignItems: 'flex-end', flexWrap: 'wrap' }}>
          <div style={styles.filterGroup}><label style={styles.label}>Class</label><select style={styles.input} value={selectedClass} onChange={e => handleClassChange(e.target.value)}><option value="">-- Select Class --</option>{classes.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
          <div style={styles.filterGroup}><label style={styles.label}>Section</label><select style={styles.input} value={selectedSection} onChange={e => handleSectionChange(e.target.value)}><option value="">-- Select Section --</option>{sections.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select></div>
          <div style={styles.filterGroup}><label style={styles.label}>Student</label><select style={styles.input} value={selectedStudent} onChange={e => setSelectedStudent(e.target.value)}><option value="">-- Select Student --</option>{students.map(s => <option key={s.id} value={s.id}>{s.profiles?.name}</option>)}</select></div>
          <button style={styles.findBtn} onClick={handleFind} disabled={!selectedStudent}>🔍 Load</button>
        </div>
      </div>
      {!searched ? (
        <div style={styles.emptyState}><div style={{ fontSize: '56px', marginBottom: '12px' }}>💬</div><div style={{ fontWeight: '600', color: '#374151', fontSize: '16px' }}>No student selected</div></div>
      ) : (
        <div style={{ marginTop: '4px' }}>
          {selectedStudentName && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '14px 16px', backgroundColor: '#f5f3ff', borderRadius: '10px', marginBottom: '16px', border: '1px solid #ddd6fe' }}>
          ...
          <div style={{ display: 'flex', gap: '10px', alignItems: 'center', marginBottom: '16px' }}>
            <input style={{ ...styles.input, flex: 1 }} placeholder="Write a feedback message..." value={newFeedback} onChange={e => setNewFeedback(e.target.value)} onKeyDown={e => e.key === 'Enter' && sendFeedback()} />
            <button style={styles.submitBtn} onClick={sendFeedback}>Send</button>
          </div>
          {loading ? <div style={{ textAlign: 'center', padding: '40px', color: '#9ca3af' }}>⏳ Loading...</div> :
            feedbacks.length === 0 ? <div style={{ textAlign: 'center', padding: '40px' }}><div style={{ fontSize: '40px' }}>📭</div><div style={{ color: '#9ca3af', fontSize: '14px' }}>No feedback yet</div></div> :
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {feedbacks.map(f => (
                  <div key={f.id} style={styles.feedbackItem}>
                    <p style={{ margin: 0, fontSize: '14px', color: '#374151', lineHeight: '1.5' }}>{f.message}</p>
                    <p style={{ margin: '6px 0 0', fontSize: '12px', color: '#9ca3af' }}>{new Date(f.created_at).toLocaleString('en-IN')}</p>
                  </div>
                ))}
              </div>
          }
...
const styles = {
  ...
  cardEditBtn: { flex: 1, padding: '7px', backgroundColor: '#22c55e', color: '#fff', border: 'none', borderRadius: '7px', cursor: 'pointer', fontSize: '12px', fontWeight: '600', textAlign: 'center' },
  cardDeleteBtn: { flex: 1, padding: '7px', backgroundColor: '#ef4444', color: '#fff', border: 'none', borderRadius: '7px', cursor: 'pointer', fontSize: '12px', fontWeight: '600', textAlign: 'center' },
  pagination: { display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '6px', marginTop: '24px', flexWrap: 'wrap' },
  pageBtn: { padding: '7px 14px', borderRadius: '7px', border: '1px solid #e5e7eb', background: '#fff', cursor: 'pointer', fontSize: '13px' },
  pageActive: { backgroundColor: '#4f46e5', color: '#fff', border: '1px solid #4f46e5' },
  modalOverlay: { position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 },
  modal: { backgroundColor: '#fff', borderRadius: '16px', padding: '28px', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' },
