...
export default function TimeTable() {
  ...
  const handleAdd = async () => {
    ...
    if (!error) {
      ...
      console.error(error)
      alert("Error adding period. Make sure 'is_lunch' and 'is_holiday' boolean columns exist in your Supabase 'timetable' table.")
    }
    setSaving(false)
  }
  ...
  // Validation Variables to control Save buttons visually
  const isAddValid = form.period_start && form.period_end && (form.is_lunch || form.is_holiday || form.subject_id);
  const isEditValid = editPeriod?.period_start && editPeriod?.period_end && (editPeriod?.is_lunch || editPeriod?.is_holiday || editPeriod?.subject_id);
        ...
        {showModal && (
          <div style={styles.modalOverlay}>
            <div style={styles.modal}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', opacity: form.is_holiday ? 0.5 : 1, pointerEvents: form.is_holiday ? 'none' : 'auto' }}>
                <div style={styles.formGroup}>
                  <label style={styles.label}>Start Time <span style={{color: '#ef4444'}}>*</span></label>
                  <input style={styles.input} type="time" value={form.period_start}
                    onChange={e => setForm({ ...form, period_start: e.target.value })} />
                </div>
                <div style={styles.formGroup}>
                  <label style={styles.label}>End Time <span style={{color: '#ef4444'}}>*</span></label>
                  <input style={styles.input} type="time" value={form.period_end}
                    onChange={e => setForm({ ...form, period_end: e.target.value })} />
                </div>
              </div>
              ...
              <div style={{ ...styles.formGroup, opacity: (form.is_lunch || form.is_holiday) ? 0.5 : 1, pointerEvents: (form.is_lunch || form.is_holiday) ? 'none' : 'auto' }}>
                <label style={styles.label}>Assign Subject <span style={{color: '#ef4444'}}>*</span></label>
                <select style={styles.input} value={form.subject_id}
                  onChange={e => setForm({ ...form, subject_id: e.target.value })}
                  disabled={form.is_lunch || form.is_holiday}
                >
              ...
              <div style={styles.formButtons}>
                ...
                <button 
                  style={{ 
                    ...styles.submitBtn, 
                    opacity: (saving || !isAddValid) ? 0.6 : 1, 
                    cursor: (saving || !isAddValid) ? 'not-allowed' : 'pointer' 
                  }} 
                  onClick={handleAdd} 
                  disabled={saving || !isAddValid}
                >
                  {saving ? '⏳ Adding...' : '＋ Save Record'}
                </button>
              </div>
            ...
            <div style={styles.modal}>
              ...
              <div style={styles.formButtons}>
                ...
                <button 
                  style={{ 
                    ...styles.submitBtn, 
                    opacity: (saving || !isEditValid) ? 0.6 : 1, 
                    cursor: (saving || !isEditValid) ? 'not-allowed' : 'pointer' 
                  }} 
...
const styles = {
  ...
  formButtons: { display: 'flex', justifyContent: 'flex-end', gap: '12px', marginTop: '12px', borderTop: '1px solid #f1f5f9', paddingTop: '20px' },
  cancelBtn: { padding: '10px 20px', borderRadius: '8px', border: '1px solid #cbd5e1', background: '#fff', cursor: 'pointer', fontSize: '14px', fontWeight: '600', color: '#475569' },
  submitBtn: { padding: '10px 24px', borderRadius: '8px', border: 'none', backgroundColor: '#2563eb', color: '#fff', cursor: 'pointer', fontSize: '14px', fontWeight: '600', transition: 'all 0.2s', boxShadow: '0 2px 4px rgba(37, 99, 235, 0.2)' },
