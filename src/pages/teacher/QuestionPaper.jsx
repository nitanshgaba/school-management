...
export default function QuestionPaper() {
  ...
  const generate = async () => {
    if (!form.class_id || !form.subject_name) { alert('⚠️ Please select a class and subject first.'); return }
    setGenerating(true)
    setGeneratedContent('')
    const prompt = `You are an experienced school teacher. Generate a complete, well-formatted question paper with the following specifications:
Subject: ${form.subject_name} | Class: ${form.class_id} | Exam: ${form.exam_type} | Difficulty: ${form.difficulty}
Total Marks: ${form.total_marks} | Time: ${form.time_minutes} minutes | ${form.topic ? `Focus: ${form.topic}` : ''}

Generate exactly these sections:
SECTION A — MCQs (${form.mcq_count} qs, 1 mark each) - include 4 options and [Answer: x]
SECTION B — TRUE/FALSE (${form.tf_count} qs, 1 mark each) - include [Answer: True/False]
SECTION C — SHORT ANSWER (${form.short_count} qs, 3 marks each) - include key points in brackets
SECTION D — LONG ANSWER (${form.long_count} qs, 5 marks each) - include key points in brackets
Format it officially. Include student instructions.`
  ...
  const printPaper = async (paper) => {
    ...
    const w = window.open('', '_blank')
    w.document.write(`
      <html><head><title>Exam Paper</title><style>
        body { font-family: 'Georgia', serif; padding: 50px; max-width: 800px; margin: 0 auto; line-height: 1.6; color: #000; }
        ...
        .meta { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; border-bottom: 1px solid #000; padding: 10px 0; margin-bottom: 20px; font-weight: bold; }
        .content { white-space: pre-wrap; font-size: 15px; }
        @media print { body { padding: 20px; } }
      </style></head><body>
      <div class="header">
        <h1>${school.school_name || 'Academic Institution'}</h1>
        <p>${school.tagline || ''}</p>
        <h2 style="margin:10px 0">${paper.subject_name} — ${paper.exam_type}</h2>
      </div>
      <div class="meta">
  ...
  const marksSum = form.mcq_count + form.tf_count + form.short_count*3 + form.long_count*5
    ...
    <div style={styles.container}>
      <div style={styles.pageHeader}>
        <div>
          ...
          <p style={styles.pageSubtitle}>Generate, manage, and publish high-quality examination papers in seconds</p>
        </div>
      </div>
      ...
      {tab === 'generate' && (
        <div style={styles.fadeIn}>
          <div style={styles.card}>
            <h2 style={styles.cardSectionTitle}>⚙️ Paper Configuration</h2>
            <div style={styles.formGrid}>
              <div style={styles.formGroup}><label style={styles.label}>Class</label>
                <select style={styles.input} value={form.class_id} onChange={e => setForm({ ...form, class_id: e.target.value })}>
                  <option value="">Select Class</option>
                  {classes.map((c, i) => <option key={i} value={c.class_id}>Class {c.classes?.name}</option>)}
                </select>
              </div>
              <div style={styles.formGroup}><label style={styles.label}>Subject</label>
              ...
              <div style={styles.formGroup}><label style={styles.label}>Difficulty</label>
                <select style={styles.input} value={form.difficulty} onChange={e => setForm({ ...form, difficulty: e.target.value })}>
                  {['Easy', 'Medium', 'Hard', 'Mixed'].map(d => <option key={d}>{d}</option>)}
                </select>
              </div>
              <div style={styles.formGroup}><label style={styles.label}>Total Marks</label>
              ...
              <div style={styles.formGroup}><label style={styles.label}>Focus Topic (Optional)</label>
            ...
            <h2 style={{ ...styles.cardSectionTitle, marginTop: '32px' }}>📊 Section Breakdown</h2>
            <div style={styles.formGrid}>
              {[{ l: 'MCQ (1m)', k: 'mcq_count' }, { l: 'T/F (1m)', k: 'tf_count' }, { l: 'Short (3m)', k: 'short_count' }, { l: 'Long (5m)', k: 'long_count' }].map(item => (
                <div key={item.k} style={styles.formGroup}>
                  <label style={styles.label}>{item.l}</label>
                  <input style={styles.input} type="number" value={form[item.k]} onChange={e => setForm({ ...form, [item.k]: parseInt(e.target.value) || 0 })} />
                </div>
              ))}
            </div>

            <div style={{ ...styles.validatorBar, backgroundColor: isMarksValid ? '#f0fdf4' : '#fff1f2', border: `1px solid ${isMarksValid ? '#bbf7d0' : '#fecaca'}` }}>
               <span style={{ fontWeight: '700', color: isMarksValid ? '#16a34a' : '#dc2626' }}>
                 {isMarksValid ? '✅ Marks distribution is correct' : `⚠️ Total adds to ${marksSum}. Must equal ${form.total_marks}`}
               </span>
            </div>

            <div style={{ display: 'flex', gap: '12px', marginTop: '24px' }}>
              <button style={styles.generateBtn} onClick={generate} disabled={generating || !isMarksValid}>
                {generating ? '⌛ Brainstorming Questions...' : '🚀 Generate Paper with AI'}
              </button>
            </div>
          </div>

          {generatedContent && (
            <div style={{ ...styles.card, marginTop: '24px', animation: 'slideUp 0.4s ease-out' }}>
               <div style={styles.outputHeader}>
                  <h3 style={{ margin: 0, fontWeight: '800' }}>📄 Preview & Edit</h3>
                  <div style={{ display: 'flex', gap: '10px' }}>
                    <button style={styles.draftBtn} onClick={() => savePaper('draft')}>💾 Save Draft</button>
                    <button style={styles.publishBtn} onClick={() => savePaper('published')}>📤 Publish for Students</button>
                  </div>
               </div>
               <textarea style={styles.editor} value={generatedContent} onChange={e => setGeneratedContent(e.target.value)} />
               <div style={styles.editorFooter}>💡 You can manually edit any question or option above before saving.</div>
            </div>
          )}
        </div>
      )}
      ...
      {tab === 'papers' && (
        <div style={styles.fadeIn}>
          {papers.length === 0 ? (
            <div style={styles.emptyState}>No papers found. Generate your first one to see it here!</div>
          ) : (
                ...
                <div key={p.id} style={styles.paperCard}>
                  <div style={styles.paperCardHeader}>
                    <div>
                      ...
                      <p style={styles.paperMeta}>Class {p.class_id} • {p.exam_type} • {p.total_marks} Marks</p>
                    </div>
                  ...
                  <div style={styles.paperCardFooter}>
                    <button style={styles.iconBtn} onClick={() => setViewPaper(p)}>👁️ View</button>
                    <button style={{ ...styles.iconBtn, color: '#4f46e5' }} onClick={() => printPaper(p)}>🖨️ Print</button>
        ...
        <div style={styles.overlay} onClick={() => setViewPaper(null)}>
          <div style={styles.modal} onClick={e => e.stopPropagation()}>
            <div style={styles.modalHeader}>
              <h3 style={{ margin: 0 }}>{viewPaper.subject_name} Review</h3>
              <button style={styles.closeBtn} onClick={() => setViewPaper(null)}>✕</button>
            </div>
            <pre style={styles.modalContent}>{viewPaper.content}</pre>
          </div>
        </div>
      )}
    </div>
...
const styles = {
  ...
  cardSectionTitle: { fontSize: '14px', fontWeight: '800', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '20px' },
  ...
  validatorBar: { marginTop: '24px', padding: '12px 20px', borderRadius: '12px', fontSize: '13px' },
  generateBtn: { flex: 1, padding: '16px', backgroundColor: '#4f46e5', color: '#fff', border: 'none', borderRadius: '12px', fontWeight: '700', cursor: 'pointer', boxShadow: '0 4px 12px rgba(79, 70, 229, 0.3)' },

  outputHeader: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' },
  editor: { width: '100%', minHeight: '600px', padding: '24px', borderRadius: '16px', border: '1px solid #e2e8f0', backgroundColor: '#fafafa', fontFamily: 'monospace', fontSize: '14px', lineHeight: '1.8', outline: 'none' },
  editorFooter: { marginTop: '12px', fontSize: '12px', color: '#94a3b8', fontStyle: 'italic' },
  
  publishBtn: { padding: '10px 20px', backgroundColor: '#10b981', color: '#fff', border: 'none', borderRadius: '10px', fontWeight: '700', cursor: 'pointer' },
  draftBtn: { padding: '10px 20px', backgroundColor: '#f1f5f9', color: '#475569', border: 'none', borderRadius: '10px', fontWeight: '700', cursor: 'pointer' },
  ...
  paperCardHeader: { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' },
  paperTitle: { fontSize: '18px', fontWeight: '800', color: '#0f172a', margin: 0 },
  ...
  paperCardFooter: { display: 'flex', gap: '10px', borderTop: '1px solid #f1f5f9', paddingTop: '16px' },
  iconBtn: { background: 'none', border: 'none', fontSize: '13px', fontWeight: '700', cursor: 'pointer', color: '#64748b' },
  ...
  modal: { backgroundColor: '#fff', borderRadius: '24px', width: '100%', maxWidth: '800px', maxHeight: '90vh', overflow: 'hidden', display: 'flex', flexDirection: 'column' },
  modalHeader: { padding: '24px', borderBottom: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' },
  modalContent: { padding: '32px', overflowY: 'auto', whiteSpace: 'pre-wrap', fontSize: '14px', lineHeight: '1.8' },
